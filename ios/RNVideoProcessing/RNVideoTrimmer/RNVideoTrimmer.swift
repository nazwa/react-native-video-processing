...
class RNVideoTrimmer: NSObject {
  ...
  @objc func crop(_ source: String, options: NSDictionary, callback: @escaping RCTResponseSenderBlock) {
    ...
    var scale : CGFloat
    let videoAspect: CGFloat = videoWidth / videoHeight
    if (CGFloat(cropAspect!) < videoAspect) {
      scale = 1280 / videoHeight
    } else {
      scale = 720 / videoWidth
    }

    let scaledVideoWidth = videoWidth * scale
    let scaledVideoHeight = videoHeight * scale

    cropOffsetX = -(720 - scaledVideoWidth)/2
    cropOffsetY = -(1280 - scaledVideoHeight)/2
    ...
    case UIImageOrientation.up:
      t1 = CGAffineTransform(translationX: scaledVideoWidth - cropOffsetX, y: 0 - cropOffsetY)
      t1 = t1.rotated(by: CGFloat(Double.pi / 2) )
      break;
    case UIImageOrientation.left:
      t1 = CGAffineTransform(translationX: scaledVideoWidth - cropOffsetX, y: scaledVideoHeight - cropOffsetY)
      t1 = t1.rotated(by: CGFloat(Double.pi) )
      break;
    ...
    case UIImageOrientation.down:
      t1 = CGAffineTransform(translationX: 0 - cropOffsetX, y: scaledVideoHeight - cropOffsetY)
      t1 = t1.rotated(by: -(CGFloat)(Double.pi / 2) )
      break;
    default:
    ...
    t1 = t1.scaledBy(x: scale, y: scale)
